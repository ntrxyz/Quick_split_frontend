...
const ExpenseDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setExpenses(prev => prev.map(exp => exp._id === expenseId ? { ...exp, ...updatedExpense } : exp));
      setIsEditing(false);
    ...
    <div className="expense-details-container">
      <div className="button-container">
        ...
        {!isEditing && (
          <div className="action-buttons">
            <Pencil className="icon-button edit-icon" onClick={() => setIsEditing(true)} />
            <Trash2 className="icon-button delete-icon" onClick={handleDelete} />
          </div>
        )}
      </div>
      ...
      {isEditing ? (
        ...
        <div className="expense-box">
          <div className="expense-field"><strong>Description:</strong> {expense.description}</div>
          <div className="expense-field"><strong>Amount:</strong> ₹{expense.amount}</div>
          <div className="expense-field"><strong>Paid By:</strong> {expense.paidBy}</div>
          <div className="expense-field"><strong>Group:</strong> {group?.name || "N/A"}</div>
          <div className="expense-field">
            <strong>Shared With:</strong>
            {expense.sharedWith?.length ? (
              <ul>{expense.sharedWith.map((email, idx) => <li key={idx}>{email}</li>)}</ul>
            ) : <p>No one</p>}
          </div>
        </div>
      )}
